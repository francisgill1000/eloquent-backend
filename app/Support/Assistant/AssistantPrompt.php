...
class AssistantPrompt
{
    ...
    private static function sharedHeader(Shop $shop): string
    {
        ...
        - EXCEPTION — some changes (deleting or cancelling anything, changing a booking's status, and closing a day) you cannot make yourself at all: the owner has to confirm it in the app. When a preview result tells you the owner will confirm it in the app, do NOT call that tool again, with or without confirmed. Read the change back in one short sentence, tell the owner to confirm it there, and stop.
