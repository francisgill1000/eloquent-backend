...
export default function Profile() {
  ...
  const shareQr = async () => {
    if (!qrTarget) return;
    const text = `Book ${shop?.name || 'us'} on AI Business Manager: ${qrTarget}`;
    if (navigator.share) {
      try { await navigator.share({ title: shop?.name || 'AI Business Manager', text, url: qrTarget }); } catch { /* dismissed */ }
    } else {
      void copyLink();
    }
  };
  ...
  const downloadQr = () => {
    ...
    ctx.fillText(shop?.name || 'AI Business Manager', W / 2, qr + pad * 2 + Math.round(footer * 0.42));
    ctx.fillStyle = '#00735c';
    ctx.font = `600 ${Math.round(qr * 0.045)}px Geist, system-ui, -apple-system, sans-serif`;
    ctx.fillText('Scan to book on AI Business Manager', W / 2, qr + pad * 2 + Math.round(footer * 0.78));
